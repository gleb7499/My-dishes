...
public class Dish {
    ...
    // Factory method
    @NonNull
    public static Dish createDish(@NonNull Dish originalDish) {
        Dish newDish = new Dish();
        newDish.setId(originalDish.getId());
        newDish.setName(originalDish.getName()); // String is immutable
        newDish.setPhotoUri(originalDish.getPhotoUri()); // String is immutable

        // Deep copy for Nutrition
        if (originalDish.getNutrition() != null) {
            newDish.setNutrition(Nutrition.createNutrition(originalDish.getNutrition()));
        }

        // Deep copy for List<Product>
        if (originalDish.getProducts() != null) {
            List<Product> newProducts = new ArrayList<>();
            for (Product product : originalDish.getProducts()) {
                if (product != null) {
                    newProducts.add(Product.createProduct(product)); // Assuming Product has a factory method
                } else {
                    newProducts.add(null);
                }
            }
            newDish.setProducts(newProducts);
        }
        return newDish;
    }
