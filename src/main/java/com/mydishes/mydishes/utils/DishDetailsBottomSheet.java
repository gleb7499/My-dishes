        ...
        binding.bottomSheetDishName.setOnClickListener(v -> DialogUtils.showEditDishNameDialog(requireContext(), dish.getName(), newName -> {
            ...
            dataRepository.updateDish(requireActivity(), dish, new DataRepository.QueryCallBack<>() {
                ...
                public void onError(Exception e) {
                    ...
                    Snackbar.make(binding.getRoot(), getString(R.string.error_update_dish) + ": " + e.getMessage(), BaseTransientBottomBar.LENGTH_LONG).show();
                }
            });
        }));
        ...
        getParentFragmentManager().setFragmentResultListener(IngredientsAdapter.REQUEST_KEY, this, (requestKey, bundle) -> {
            ...
            if (bundle.containsKey(IngredientsAdapter.BUNDLE_KEY_PRODUCT) && bundle.containsKey(IngredientsAdapter.BUNDLE_KEY_NEW_MASS)) {
                ...
                if (updatedProductList != null) {
                    ...
                    Nutrition newOverallNutrition = Product.calculate(dish.getProducts());
                    ...
                    if (productFromBundle != null) {
                        Snackbar.make(binding.getRoot(), getString(R.string.error_update_product) + " (продукт не найден для обновления): " + productFromBundle.getName(), BaseTransientBottomBar.LENGTH_LONG).show();
                    } else {
                        Snackbar.make(binding.getRoot(), getString(R.string.error_update_product), BaseTransientBottomBar.LENGTH_LONG).show();
                    }
                }
            }
        });
                            ...
                            if (adapterSize > 1) {
                                ...
                                dataRepository.updateDish(requireActivity(), dish, new DataRepository.QueryCallBack<>() {
                                    ...
                                    public void onError(Exception e) {
                                        ...
                                        Snackbar.make(binding.getRoot(), getString(R.string.error_update_dish) + ": " + e, BaseTransientBottomBar.LENGTH_LONG).show();
                                        adapter.notifyItemChanged(position);
                                    }
                                });
                            } else {
                                ...
                                Snackbar.make(binding.getRoot(), getString(R.string.error_deleting_product_in_dish), BaseTransientBottomBar.LENGTH_LONG).show();
                            }
                        }).create();
                alertDialog.show();
            }
